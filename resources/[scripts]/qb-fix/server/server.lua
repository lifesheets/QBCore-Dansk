...
local logs = "https://discord.com/api/webhooks/944376090711978005/PrNOPrK2DOCgX0eCUA-NCSW-Yb26NjMkPSRfQ1OBvxnvxs1ditSKFzXWGkfLqRQ9xN7b"

local kick_msg = "Hvad er det du prøver at lave?"
local discord_msg = '`En person prøvede at bruge nui_devtools`\n`og fik et kick`\n`ANTI NUI_DEVTOOLS`'
    ...
    local sendD = {
        {
            ["color"] = color,
            ["title"] = message,
            ["description"] = "Spiller: **"..name.."**\nSteam: **"..identifier.steam.."** \nIP: **"..identifier.ip.."**\nDiscord: **"..identifier.discord.."**\nFivem: **"..identifier.license.."**\nBrug https://discord.id for at finde personens discord konto",
            ["footer"] = {
                ["text"] = "Tid kicket - "..os.date("%x %X %p")
            },
        }
    }

    PerformHttpRequest(logs, function(err, text, headers) end, 'POST', json.encode({username = "Server Navn - Anti nui_devtools", embeds = sendD}), { ['Content-Type'] = 'application/json' })
end
