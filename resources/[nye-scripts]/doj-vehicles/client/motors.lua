...
--========================================================== Turbo
RegisterNetEvent('doj:client:applyMotor', function()
	local playerPed	= PlayerPedId()
    local coords	= GetEntityCoords(playerPed)
    if IsPedSittingInAnyVehicle(playerPed) then
		QBCore.Functions.Notify("Stig ud af køretøjet for at installere motoren", "error", 3500)
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 3.5) then
			...
			if Config.isVehicleOwned then
					...
					if finished ~= 100 then
						...
						if finished2 ~= 100 then
							...
							if finished3 ~= 100 then
								...
								CurrentVehicleData = QBCore.Functions.GetVehicleProperties(vehicle)
								TriggerServerEvent('updateVehicle', CurrentVehicleData)
								TriggerServerEvent('doj:server:removeMotor')
							end
						end
					end
				elseif Config.skillbarTurbo == "np-skillbar" then
				...
				elseif Config.skillbarTurbo == "qb-skillbar" then
					...
					Skillbar.Start({
						...
						QBCore.Functions.Notify("Success! Installing Motor", "success", 3500)
						FreezeEntityPosition(playerPed, true)
						time = math.random(7000,10000)
						TriggerEvent('pogressBar:drawBar', time, 'Installerer Motor')
						Wait(time)
						SetVehicleModKit(vehicle, 0)
						ToggleVehicleMod(vehicle, 11, true)
						SetVehicleDoorShut(vehicle, 4, false)
						...
						QBCore.Functions.Notify("Motor installation fejlede!", "error", 3500)
						ClearPedTasks(playerPed)
					end)
				end
			end
		...
		QBCore.Functions.Notify("Der er intet køretøj i nærheden", "error", 3500)
	end
end)
