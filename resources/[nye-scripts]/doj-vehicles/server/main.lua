...
--========================================================== Motor
RegisterNetEvent('doj:server:removeMotor', function()
    local Player = QBCore.Functions.GetPlayer(source)
    Player.Functions.RemoveItem('motor', 1)
    TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items['motor'], "remove", 1)
    TriggerClientEvent('QBCore:Notify', source, "Motor er blevet installeret", 'success')
end)

QBCore.Functions.CreateUseableItem("motor", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
	TriggerClientEvent("doj:client:applyMotor", source) 
end)