...
AddEventHandler('mt-hunting:server:ComprarSniper', function(data)
    ...
    if Player.Functions.RemoveMoney('bank', price) then
        ...
        QBCore.Functions.Notify('Ikke nok penge', 'error') -- [text] = message, [type] = primary | error | success, [length] = time till fadeout
    end
end)
...
RegisterNetEvent('mt-hunting:server:Vender', function()
    ...
    if xItem ~= nil then
        ...
        Player.Functions.AddMoney("cash", price, "sold-meat")
            TriggerClientEvent('QBCore:Notify', src, "Du solgte noget kød for "..price.." DKK")
                end
            end
        end
    else
        TriggerClientEvent('QBCore:Notify', src, "Du har ikke noget at sælge..")
    end
end)
