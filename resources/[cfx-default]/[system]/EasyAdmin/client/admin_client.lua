...
Citizen.CreateThread(function()
	RegisterNetEvent("EasyAdmin:requestCleanup", function(type)
		...
		TriggerClientEvent('QBCore:Notify', string.format(GetLocalisedText("finishedcleaning"), GetLocalisedText(type)))
	end)
end)
Citizen.CreateThread( function()
	while true do
...
function spectatePlayer(targetPed,target,name)
	...
	if(enable)then
		...
		TriggerClientEvent('QBCore:Notify', string.format(GetLocalisedText("spectatingUser"), name))
	else
		if oldCoords then
			RequestCollisionAtCoord(oldCoords.x, oldCoords.y, oldCoords.z)
			Wait(500)
		...
		if vehicleInfo.netId and vehicleInfo.seat then
			...
			if DoesEntityExist(vehicle) then
				if IsVehicleSeatFree(vehicle, vehicleInfo.seat) then
					...
					TriggerClientEvent('QBCore:Notify', GetLocalisedText("spectatevehicleseatoccupied"))
				end
			else
				TriggerClientEvent('QBCore:Notify', GetLocalisedText("spectatenovehiclefound"))
			end
