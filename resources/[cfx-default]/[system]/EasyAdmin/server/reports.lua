        ...
        if GetConvar("ea_enableCallAdminCommand", "true") == "true" then
            if args[1] then
                ...
                if cooldowns[source] and cooldowns[source] > (time - cooldowntime) then
                    TriggerClientEvent('QBCore:Notify', source, GetLocalisedText("waitbeforeusingagain"), "error")
                    return
                end
...
--                    TriggerClientEvent('QBCore:Notify', i, notificationText)
                end
                ...
                TriggerClientEvent('QBCore:Notify', source, GetLocalisedText("admincalled"), "success")
                
                time = os.time()
                cooldowns[source] = time
            else
                TriggerClientEvent('QBCore:Notify', source, GetLocalisedText("invalidreport"))
            end
        end
    end, false)
            ...
            if args[2] then
                ...
                if id and valid then
                    ...
                    if addReport then
                        ...
                        TriggerClientEvent('QBCore:Notify', source, GetLocalisedText("successfullyreported"))
                        ...
                        TriggerClientEvent('QBCore:Notify', source, GetLocalisedText("alreadyreported"))
                    end
                else
                    ...
                    TriggerClientEvent('QBCore:Notify', source, GetLocalisedText("reportedusageerror"))
                end
            else
                TriggerClientEvent('QBCore:Notify', source, GetLocalisedText("invalidreport"))
            end
	...
	RegisterServerEvent("EasyAdmin:ClaimReport", function(reportId)
		if DoesPlayerHavePermission(source, "player.reports.claim") then
			if not reports[reportId].claimed then
				...
				TriggerClientEvent('QBCore:Notify', source, GetLocalisedText("reportalreadyclaimed"))
			end
		end
	end)
