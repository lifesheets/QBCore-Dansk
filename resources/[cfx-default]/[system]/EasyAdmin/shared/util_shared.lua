	...
	if level == 1 and loglevel >= level then -- ERROR Loglevel
		...
		if IsDuplicityVersion() then
			for i,k in pairs(GetOnlineAdmins()) do
				TriggerClientEvent('QBCore:Notify', i, string.gsub(msg, "%^%d", ""))
			end
		else
			TriggerClientEvent('QBCore:Notify', string.gsub(msg, "%^%d", ""))
		end
...
function copyToClipboard(text)
	...
	TriggerClientEvent('QBCore:Notify', GetLocalisedText("copiedtoclipboard"))
end
