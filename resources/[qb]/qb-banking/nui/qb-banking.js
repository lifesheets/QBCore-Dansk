...
function setupSavingsMenu(data, name)
{
    ...
    $.each(statement2, function (index, statement) {
        ...
        $("#savingsStatementContents").append('<tr class="statement"><td><small>' + statement.date + '</small></td><td><small>' + statement.type + '</small></td><td class="text-center text-danger"><small>' + withdraw + ' DKK</small></td><td class="text-center text-success"><small>' + deposit + ' DKK</small></td><td class="text-center"><small>' + balance + '</small></td></tr>');
...
function populateBanking(data)
{
    ...
    if(data.cardInformation !== undefined) {
        if (data.cardInformation.cardLocked == true) {
            ...
            $("#debitCardStatus").html('<div class="card-header">Card Locked</div><div class="card-body">Dit kort er i øjeblikket lukket.</div><div class="card-footer"><button class="btn btn-primary btn-block" id="unLockCard">Lås op/ophæv blokering af kort</button></div>');
        } else {
            $("#debitCardStatus").removeClass('bg-danger');
        $("#debitCardStatus").addClass('bg-success');
        $("#debitCardStatus").html('<div class="card-header">Card Unlocked</div><div class="card-body">Dit kort er i øjeblikket aktivt.</div><div class="card-footer"><button class="btn btn-primary btn-block" id="lockCard">Lås/bloker kort</button></div>');
        }
...
$(function() {
    ...
    $("#updateNewPin").click(function() {
        ...
        if(newPin !== null && newPin !== undefined && newPin.replace(/[^0-9]/g,"").length === 4) {
            ...
            $("#newPinReqMsg").html('Du skal angive et 4-cifret tilfældigt tal for at opdatere din pinkode.')
        }
    ...
    $("#initiateWithdraw").click(function() {
        ...
        if(amount !== undefined && amount > 0) {
            ...
            $("#withdrawErrorMsg").html('Der opstod en fejl under behandlingen af ​​din udbetaling, enten er beløbet ikke blevet indtastet, eller det er ikke et positivt tal');
        }
    });
    ...
    $("#initiateWithdrawATM").click(function() {
        ...
        if (currentLimit + parseInt(amount) <= Config.ATMTransLimit) {
            if(amount !== undefined && amount > 0) {
                ...
                $("#withdrawATMError").css({"display":"none"});
                $("#withdrawATMErrorMsg").html('Du har hævet ' + amount + ' DKK fra din account.');
                currentLimit = currentLimit + parseInt(amount);
            } else {
                // Error doing withdraw
                $("#withdrawATMErrorMsg").removeClass('alert-success').addClass('alert-danger');
                $("#withdrawATMError").css({"display":"block"});
                $("#withdrawATMErrorMsg").html('Der opstod en fejl under behandlingen af ​​din udbetaling, enten er beløbet ikke blevet indtastet, eller det er ikke et positivt tal');
            }
        } else {
            $("#withdrawATMErrorMsg").removeClass('alert-success').addClass('alert-danger');
            $("#withdrawATMError").css({"display":"block"});
            $("#withdrawATMErrorMsg").html('Beklager, du har nået den daglige udbetalingsgrænse på ' + Config.ATMTransLimit + ' DKK for dagen.');
        }
    });
    ...
    $("#initiateDeposit").click(function() {
        ...
        if(amount !== undefined && amount > 0) {
            ...
            $("#depositErrorMsg").html('Der opstod en fejl under behandlingen af ​​din indbetaling, enten er beløbet ikke blevet indtastet, eller det er ikke et positivt tal');
        }
    });
    ...
    $("#processCard").click(function() {
        ...
        if(pinValue !== null && pinValue !== undefined && pinValue.replace(/[^0-9]/g,"").length === 4) {
            ...
            $("#pinCreatorErrorMsg").html('Der opstod en fejl med pinkoden, du forsøgte at oprette. Lav venligst et tilfældigt tal, som er 4 cifret.');
        }
    ...
    $("#initiateTransfer").click(function() {
        ...
        if(amount !== undefined && amount !== null && amount > 0 && sortcode !== undefined && sortcode !== null && sortcode > 0 && account !== undefined && account !== null && account > 0) {
            ...
            $("#transferError").css({"display":"block"});
            $("#transferErrorMsg").html('Der opstod en fejl med de oplysninger, du har indtastet. Sørg for, at kontonummer, sorteringskode og beløb er korrekt udfyldt.');
        }
    ...
    $("[data-action=ATMwithdraw]").click(function() {
        ...
        if (currentLimit + parseInt(amount) <= Config.ATMTransLimit) {
            ...
            $("#successMessageATM").html('Beklager, du har nået den daglige udbetalingsgrænse på ' + Config.ATMTransLimit + ' DKK for dagen.');
        }
    });
        ...
        if(number == "ENTER") {
            ...
            if (enteredPin !== null && enteredPin !== undefined || enteredPin.replace(/[^0-9]/g,"").length !== 4) {
                    if(clientPin == enteredPin) {
                        ...
                        $("#pinErrorMsg").html('Du har indtastet en forkert pinkode.');
                        $("#pinErrorDiv").css({"display":"block"});
                    }
            } else {
                $("#pinErrorMsg").html('Du skal indtaste en pinkode, som er 4 cifre lang.');
                $("#pinErrorDiv").css({"display":"block"});
            }
