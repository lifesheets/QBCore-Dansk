...
RegisterNetEvent('qb-banking:createBankCard', function(pin)
    ...
    TriggerClientEvent('QBCore:Notify', src, 'Du har bestilt et Debit Kort.', 'success')
...
RegisterNetEvent('qb-banking:doQuickDeposit', function(amount)
    ...
    if tonumber(amount) <= currentCash then
        ...
        if bank then
            TriggerClientEvent('qb-banking:openBankScreen', src)
            TriggerClientEvent('qb-banking:successAlert', src, 'Du har foretaget en kontant indbetaling på '..amount..' DKK.')
...
RegisterNetEvent('qb-banking:doQuickWithdraw', function(amount, branch)
    ...
    if tonumber(amount) <= currentCash then
        ...
        if cash then
            TriggerClientEvent('qb-banking:openBankScreen', src)
            TriggerClientEvent('qb-banking:successAlert', src, 'Du foretog en kontant udbetaling på '..amount..' DKK.')
...
RegisterNetEvent('qb-banking:updatePin', function(pin)
    if pin ~= nil then
        ...
        TriggerClientEvent('qb-banking:openBankScreen', src)
        TriggerClientEvent('qb-banking:successAlert', src, 'Du har opdateret dit debitkort pinkode.')
    end
end)
...
RegisterNetEvent('qb-banking:savingsDeposit', function(amount)
    ...
    if tonumber(amount) <= currentBank then
        ...
        local savings = savingsAccounts[xPlayer.PlayerData.citizenid].AddMoney(tonumber(amount), 'Nuværende Konto til Opsparing')
        while bank == nil do Wait(0) end
        while savings == nil do Wait(0) end
        TriggerClientEvent('qb-banking:openBankScreen', src)
        TriggerClientEvent('qb-banking:successAlert', src, 'Du har foretaget et opsparingsindskud på '..tostring(amount)..' DKK.')
...
RegisterNetEvent('qb-banking:savingsWithdraw', function(amount)
    ...
    if tonumber(amount) <= currentSavings then
        local savings = savingsAccounts[xPlayer.PlayerData.citizenid].RemoveMoney(tonumber(amount), 'Opsparing til Nuværende Konto')
        local bank = xPlayer.Functions.AddMoney('bank', tonumber(amount), 'banking-quick-withdraw')
        while bank == nil do Wait(0) end
        while savings == nil do Wait(0) end
        TriggerClientEvent('qb-banking:openBankScreen', src)
        TriggerClientEvent('qb-banking:successAlert', src, 'Du har hævet '..tostring(amount)..' successfully.')
...
RegisterNetEvent('qb-banking:createSavingsAccount', function()
    ...
    TriggerClientEvent('qb-banking:openBankScreen', src)
    TriggerClientEvent('qb-banking:successAlert', src, 'Du har åbnet en opsparingskonto.')
				...
				if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(id))) < distance then
					if xPlayer.Functions.RemoveMoney('cash', amount) then
						if xReciv.Functions.AddMoney('cash', amount) then
							TriggerClientEvent('QBCore:Notify', src, "Du gav " .. tostring(amount) .. ' DKK til ID ' .. tostring(id) .. '', "success")
							TriggerClientEvent('QBCore:Notify', id, "Du fik " .. tostring(amount) .. ' DKK fra ID ' .. tostring(src), "success")
					...
					TriggerClientEvent('QBCore:Notify', src, "Du er for langt væk fra personen.", "error")
				end
			else
				TriggerClientEvent('QBCore:Notify', src, "Du er død.", "error")
			end
		else
			TriggerClientEvent('QBCore:Notify', src, "Forkert ID.", "error")
		end
	else
		TriggerClientEvent('QBCore:Notify', src, "Brug /givecash [ID] [AMOUNT]", "error")
	end
end)
