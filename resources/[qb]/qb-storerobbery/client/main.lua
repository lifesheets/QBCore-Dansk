            ...
            for safe,_ in pairs(Config.Safes) do
                ...
                if dist < 3 then
					...
					local s1, s2 = GetStreetNameAtCoord(pos.x, pos.y, pos.z)
                                    ...
                                    QBCore.Functions.Notify("Ikke Nok Politi (".. Config.MinimumStoreRobberyPolice .." Påkrævet)", "error")
                                end
                            end
                        else
...
RegisterNUICallback('success', function()
    if currentRegister ~= 0 then
        ...
        QBCore.Functions.Progressbar("search_register", "Tømmer Kasseapperatet..", lockpickTime, false, true, {
...
RegisterNUICallback('fail', function()
    ...
    if (IsWearingHandshoes() and math.random(1, 100) <= 25) then
        local pos = GetEntityCoords(PlayerPedId())
        TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
        QBCore.Functions.Notify("Du ødelagde lockpicken")
    end
    lockpick(false)
end)
