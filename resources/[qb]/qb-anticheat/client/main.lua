        ...
        if group == Config.Group and LocalPlayer.state.isLoggedIn then
            if not IsDecorating then
                if not IsEntityVisible(ped) then
                    ...
                    TriggerEvent('QBCore:Notify', "DRP-ANTICHEAT: Du var usynlig og blev lavet synlig igen!")
                    TriggerServerEvent("qb-log:server:CreateLog", "anticheat", "Made player visible", "green", "** @everyone " ..GetPlayerName(player).. "** was invisible and has been made visible again by DRP-Anticheat")
                end
            end
        end
    end
