...
local function smashVitrine(k)
    ...
    elseif math.random(1, 100) <= 5 and IsWearingHandshoes() then
        TriggerServerEvent("evidence:server:CreateFingerDrop", plyCoords)
        QBCore.Functions.Notify("Du har efterladt et fingeraftryk på glasset", "error")
    end
    smashing = true
    QBCore.Functions.Progressbar("smash_vitrine", "Stocking a display", Config.WhitelistedWeapons[pedWeapon]["timeOut"], false, true, {
        ...
        TriggerServerEvent('police:server:policeAlert', 'Røveri i gang')
        smashing = false
...
CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName("Vangelico Smykker")
                ...
                if dist < 30 then
                            ...
                            if IsControlJustPressed(0, 38) then
                                QBCore.Functions.TriggerCallback('qb-jewellery:server:getCops', function(cops)
                                    if cops >= Config.RequiredCops then
                                        ...
                                        QBCore.Functions.Notify('Ikke Nok Politi ('.. Config.RequiredCops ..') Påkrævet', 'error')
                                    end
                                end)
                            end
                        end
                    ...
                    if storeDist < 2 then
                        if not firstAlarm then
                            if validWeapon() then
                                TriggerServerEvent('police:server:policeAlert', 'Mistænkelig Aktivitet')
                                firstAlarm = true
                            end
                        end
                    end
