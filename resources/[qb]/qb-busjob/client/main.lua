            ...
            if dist < 20 then
                ...
                if dist < 5 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        QBCore.Functions.Notify('Folk Gik Af', 'success')
                        if NpcData.DeliveryBlip ~= nil then
                            RemoveBlip(NpcData.DeliveryBlip)
                        end
...
RegisterNetEvent("qb-busjob:client:TakeVehicle", function(data)
    ...
    if(BusData.Active) then
        QBCore.Functions.Notify('Du kan kun have en bus ude af gangen', 'error')
        return
    else
...
RegisterNetEvent('qb-busjob:client:DoBusNpc', function()
    if whitelistedVehicle() then
        if not NpcData.Active then
            ...
            QBCore.Functions.Notify('Kør til det næste busstop', 'success')
           ...
           QBCore.Functions.Notify('Du kører allerede en bus')
        end
    else
        QBCore.Functions.Notify('Du Er Ikke I En Bus')
    end
end)
                ...
                if vehDist < 30 then
                    ...
                    if vehDist < 1.5 then
                        if whitelistedVehicle() then
                            ...
                            DrawText3D(Config.Location.x, Config.Location.y, Config.Location.z + 0.3, '[E] Stop Arbejde')
                            if IsControlJustReleased(0, 38) then
                                if (not NpcData.Active or NpcData.Active and NpcData.NpcTaken == false)then
                                    ...
                                    QBCore.Functions.Notify("Du skal have alle passagererne af før du kan stoppe med at arbejde", "error")
                                end
                            end
                        else
                            DrawText3D(Config.Location.x, Config.Location.y, Config.Location.z + 0.3, '[E] Job Køretøjer')
                            if IsControlJustReleased(0, 38) then
                                BusGarage()
                            end
                        end
