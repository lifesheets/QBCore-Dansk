...
local function NoDamage()
    local NoDamage = {
        ...
        {
            header = "Tilbage",
            txt = "Der Er Ingen Skade Til Denne Part!",
            params = {
                event = "qb-mechanicjob:client:PartsMenu",
            }
        },
...
local function RepairPart(part)
    ...
    QBCore.Functions.TriggerCallback('qb-inventory:server:GetStashItems', function(StashItems)
        ...
        if hasitem and countitem >= PartData.costs then
            ...
            QBCore.Functions.Notify('Der er ikke materialer nok i skabet', 'error')
        end
    end, "mechanicstash")
end
...
RegisterNetEvent('qb-vehicletuning:client:RepaireeePart', function(part)
    ...
    QBCore.Functions.Notify(""..Config.ValuesLabels[part].." Er Repareret!")
end)
            ...
            if #(pos - vehpos) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                        ...
                        QBCore.Functions.Progressbar("repair_advanced", "Reparerer Køretøjet", lockpickTime, false, true, {
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
...
CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName("Mekaniker")
            ...
            if PlayerJob.name == "mechanic" then
                ...
                if onDuty then
                    if VehicleDistance < 20 then
                        ...
                        if VehicleDistance < 1 then
                            ...
                            if InVehicle then
                                DrawText3Ds(Config.Locations["vehicle"].x, Config.Locations["vehicle"].y, Config.Locations["vehicle"].z, '[E] Skjul Køretøj')
                                if IsControlJustPressed(0, 38) then
                                    DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
                                end
                            else
                                DrawText3Ds(Config.Locations["vehicle"].x, Config.Locations["vehicle"].y, Config.Locations["vehicle"].z, '[E] Få Køretøj')
                                if IsControlJustPressed(0, 38) then
                                    if IsControlJustPressed(0, 38) then
                                        VehicleList()
                                    end
                ...
                if OnDutyDistance < 20 then
                    ...
                    if OnDutyDistance < 1 then
                        if onDuty then
                            DrawText3Ds(Config.Locations["duty"].x, Config.Locations["duty"].y, Config.Locations["duty"].z, "[E] Af Job")
                        else
                            DrawText3Ds(Config.Locations["duty"].x, Config.Locations["duty"].y, Config.Locations["duty"].z, "[E] På Job")
                        end
                        if IsControlJustReleased(0, 38) then
                            TriggerServerEvent("QBCore:ToggleDuty")
                        end
                    ...
                    for k, v in pairs(Config.Plates) do
                        if v.AttachedVehicle == nil then
                            ...
                            if PlateDistance < 3 then
                                inRange = true
                                DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "[E] Åben Menu")
                                if IsControlJustPressed(0, 38) then
                                    OpenMenu()
                                end
                            end
