...
function takeHostage()
	...
	if closestPlayer ~= nil and closestPlayer ~= -1 then
		...
		for i=1, #hostageAllowedWeapons do
			if HasPedGotWeapon(GetPlayerPed(-1), GetHashKey(hostageAllowedWeapons[i]), false) then
				if GetAmmoInPedWeapon(GetPlayerPed(-1), GetHashKey(hostageAllowedWeapons[i])) > 0 then
					...
					TriggerClientEvent('QBCore:Notify', src, 'Du har ingen ammunition!', 'error')
					return			
				end	
			else
				NoWeapon = true	
		...
		if NoWeapon then
			TriggerClientEvent('QBCore:Notify', src, 'Du kan ikke bruge dette våben til at tage gidsler!', 'error')
			return
		end
...
--	else
--	    TriggerClientEvent('QBCore:Notify', src, 'Ingen personer i nærheden!', 'error')
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"[G] Giv Slip, [H] Dræb")
...
TriggerEvent('chat:addSuggestion', '/tg', 'Tag en person som gidsel', {})