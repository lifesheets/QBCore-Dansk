...
RegisterNetEvent('qb-carwash:client:washCar', function()
    ...
    washingVehicle = true
    QBCore.Functions.Progressbar("search_cabin", "Køretøjet bliver vasket ..", math.random(4000, 8000), false, true, {
        ...
        QBCore.Functions.Notify("Vask Annulleret ..", "error")
        washingVehicle = false
    end)
end)
                    ...
                    if dist <= 7.5 then
                        if Driver == PlayerPed then
                            if not washingVehicle then
                                DrawText3Ds(Config.CarWash[k]["coords"]["x"], Config.CarWash[k]["coords"]["y"], Config.CarWash[k]["coords"]["z"], '~g~E~w~ - Vask bil ('..Config.DefaultPrice..' DKK)')
                                ...
                                DrawText3Ds(Config.CarWash[k]["coords"]["x"], Config.CarWash[k]["coords"]["y"], Config.CarWash[k]["coords"]["z"], 'Bilvasken er ikke tilgængelig ..')
                            end
                        end
                    end
                end
