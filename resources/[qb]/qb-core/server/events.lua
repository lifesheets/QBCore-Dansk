...
local function onPlayerConnecting(name, setKickReason, deferrals)
    ...
    deferrals.update(string.format('Hej %s, Validerer Rockstar Licens', name))
    ...
    deferrals.update(string.format('Hej %s, Tjekker om du er bannet.', name))
    ...
    if not license then
        deferrals.done('Ingen Valid Rockstar Licens Fundet')
    elseif isBanned then
        deferrals.done(Reason)
    elseif isLicenseAlreadyInUse and QBCore.Config.Server.CheckDuplicateLicense then
        deferrals.done('Duplikeret Rockstar Licens Fundet (Rejoin)')
    elseif isWhitelist and not whitelisted then
        deferrals.done('Du er ikke whitelisted')
    else
        deferrals.done()
...
RegisterNetEvent('QBCore:Server:CloseServer', function(reason)
    ...
    if QBCore.Functions.HasPermission(src, 'admin') then
        reason = reason or 'Ingen grund givet'
        ...
        QBCore.Functions.Kick(src, 'Du har ikke adgang til dette..', nil, nil)
    end
end)
