...
QBCore.Commands.Add('addpermission', 'Give Player Permissions (God Only)', { { name = 'id', help = 'ID på spiller' }, { name = 'permission', help = 'Permission level' } }, true, function(source, args)
...
QBCore.Commands.Add('removepermission', 'Remove Players Permissions (God Only)', { { name = 'id', help = 'ID på spiller' }, { name = 'permission', help = 'Permission level' } }, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(tonumber(args[1]))
    local permission = tostring(args[2]):lower()
    if Player then
...
QBCore.Commands.Add('openserver', 'Åben serveren for alle (Kun Ejer)', {}, false, function(source)
    if not QBCore.Config.Server.Closed then
        TriggerClientEvent('QBCore:Notify', source, 'Serveren er allerede åben', 'error')
        return
    end
    if QBCore.Functions.HasPermission(source, 'god') then
        QBCore.Config.Server.Closed = false
    else
        QBCore.Functions.Kick(source, 'Du har ikke adgang til dette..', nil, nil)
    end
end, 'god')

QBCore.Commands.Add('closeserver', 'Luk serveren så kun folk med adgang kan joine (Kun Ejer)', { { name = 'reason', help = 'Reason for closing it (optional)' } }, false, function(source, args)
    if QBCore.Config.Server.Closed then
        TriggerClientEvent('QBCore:Notify', source, 'Serveren er allerede lukket', 'error')
        return
    end
    if QBCore.Functions.HasPermission(source, 'god') then
        local reason = args[1] or 'Ingen grund givet'
...
QBCore.Commands.Add('car', 'Spawn Et Køretøj', { { name = 'model', help = 'Spawnkode på bil' } }, true, function(source, args)
    TriggerClientEvent('QBCore:Command:SpawnVehicle', source, args[1])
end, 'admin')

QBCore.Commands.Add('dv', 'Slet Køretøj', {}, false, function(source)
    TriggerClientEvent('QBCore:Command:DeleteVehicle', source)
end, 'admin')

-- Money

QBCore.Commands.Add('givemoney', 'Giv En Spiller Penge', { { name = 'id', help = 'Spiller ID' }, { name = 'moneytype', help = 'Type af penge (cash, bank, crypto)' }, { name = 'amount', help = 'Antal af penge' } }, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(tonumber(args[1]))
    if Player then
...
QBCore.Commands.Add('setmoney', 'Sæt En Spillers Pengebeløb', { { name = 'id', help = 'Spiller ID' }, { name = 'moneytype', help = 'Type af penge (cash, bank, crypto)' }, { name = 'amount', help = 'Antal af penge' } }, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(tonumber(args[1]))
    if Player then
...
QBCore.Commands.Add('job', 'Tjek Dit Job', {}, false, function(source)
...
QBCore.Commands.Add('setjob', 'Sæt En Spillers Job', { { name = 'id', help = 'Spiller ID' }, { name = 'job', help = 'Job navn' }, { name = 'grade', help = 'Grad' } }, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(tonumber(args[1]))
    if Player then
...
QBCore.Commands.Add('gang', 'Tjek Din Bande', {}, false, function(source)
...
QBCore.Commands.Add('setgang', 'Sæt En Spillers Bande', { { name = 'id', help = 'Spiller ID' }, { name = 'gang', help = 'Navn på bande' }, { name = 'grade', help = 'Grad' } }, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(tonumber(args[1]))
    if Player then
...
QBCore.Commands.Add('clearinv', 'Ryd En Spillers Inventory', { { name = 'id', help = 'Spiller ID' } }, false, function(source, args)
...
QBCore.Commands.Add('ooc', 'OOC Chat Besked', { { name = 'besked', help = 'Besked' } }, false, function(source, args)
...
QBCore.Commands.Add('me', 'Lav en handling', {{name = 'message', help = 'Handling'}}, false, function(source, args)
