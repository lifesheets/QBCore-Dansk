                            ...
                            QBCore.Functions.TriggerCallback('garbagejob:server:NewShift', function(shouldContinue, firstStop, totalBags)
                                if shouldContinue then
                                    ...
                                    QBCore.Functions.SpawnVehicle("trash2", function(veh)
                                        ...
                                        isWorking = true
                                        SetGarbageRoute()
                                        QBCore.Functions.Notify("Du har "..Config.TruckPrice.." DKK, depositum betalt!")
                                        QBCore.Functions.Notify("Du er begyndt at arbejde, placering markeret på GPS!")
                                        Wait(10)
                                        -- Run the Work Loop to check for Garbo Bags.
                                        RunWorkLoop()
                                    end, coords, true)
                                else
                                    QBCore.Functions.Notify("Du har ikke penge nok til depositum.. Indbetalingsomkostninger er "..Config.TruckPrice.. " DKK")
                                end
                            end)
                        end
                    end
