            ...
            if result then
                ...
                if vehLockStatus == 1 then
                    ...
                    if (GetVehicleDoorLockStatus(veh) == 2) then
                        ...
                        QBCore.Functions.Notify("Der gik noget galt med låsesystemet!")
                    end
                else
                    Wait(750)
                    ClearPedTasks(ped)
                ...
                QBCore.Functions.Notify('Du har ikke nøglerne til køretøjet..', 'error')
            end
        end, plate)
    end
end
        ...
        if math.random() <= chance then
            ...
            if closestPed ~= nil then
                ...
                if IsPedInAnyVehicle(ped) then
                    ...
                    local msg = "Forsøg på køretøjstyveri ved " .. streetLabel .. ". Køretøj: " .. Name .. ", Nummerplade: " .. modelPlate
                    local alertTitle = "Forsøg på køretøjstyveri ved"
...
local function lockpickFinish(success)
    ...
    if success then
        ...
        QBCore.Functions.Notify('Åbnede Dør!', 'success')
        ...
        PoliceCall()
        TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
        QBCore.Functions.Notify('Nogen ringede til Politiet!', 'error')
    end
...
local function Hotwire()
    if not HasVehicleKey then
        ...
        IsHotwiring = true
        lockpickedPlate = nil
        local hotwireTime = math.random(5000, 20000)
...
local function RobVehicle(target)
    ...
    QBCore.Functions.Progressbar("rob_keys", "Forsøger Røveri..", 6000, false, true, {}, {}, {}, {}, function()
        local chance = math.random()
        if chance <= Config.RobberyChance then
            ...
            TaskReactAndFleePed(target, PlayerPedId())
            TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
            QBCore.Functions.Notify('De Ringede Til Politiet!', 'error')
            Wait(10000)
            IsRobbing = false
        end
    end)
...
RegisterNetEvent('vehiclekeys:client:GiveKeys', function(target)
    ...
    if vehicles then
        ...
        QBCore.Functions.Notify('Du skal være i et køretøj for at give en nøgle', 'error')
    end
end)
...
RegisterKeyMapping('togglelocks', 'Lås/Oplås Køretøj', 'keyboard', 'L')
