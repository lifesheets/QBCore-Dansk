...
local WebHook = "https://discord.com/api/webhooks/931907866267365386/HoIllRV6bmDeb-xSx6qhTl12GnqrTNRO78vzDinDH6alL-sNtLAB8lECtFeJVrjPL3vP"
...
QBCore.Functions.CreateCallback('qb-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    ...
    if SenderPly and Config.BillingCommissions[society] then
        ...
        invoiceMailData = {
            ...
            subject = 'Kommission Modtaget',
            message = string.format('Du modtog en provisionscheck på %s DKK, da %s %s betalte en regning på %s DKK.', commission, Ply.PlayerData.charinfo.firstname, Ply.PlayerData.charinfo.lastname, amount)
        }
    elseif not SenderPly and Config.BillingCommissions[society] then
        invoiceMailData = {
            sender = 'Faktureringsafdeling',
            subject = 'Regning Betalt',
            message = string.format('%s %s betalte en regning på %s DKK', Ply.PlayerData.charinfo.firstname, Ply.PlayerData.charinfo.lastname, amount)
        }
    end
    Ply.Functions.RemoveMoney('bank', amount, "betalt faktura") --paid-invoice
...
QBCore.Functions.CreateCallback('qb-phone:server:ScanPlate', function(source, cb, plate)
    ...
    if plate ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', src, 'Intet Køretøj I Nærheden', 'error')
        cb(nil)
    end
end)
...
QBCore.Functions.CreateCallback('qb-phone:server:CanTransferMoney', function(source, cb, amount, iban)
    ...
    if (Player.PlayerData.money.bank - amount) >= 0 then
        local query = '%"konto":"' .. iban .. '"%' -- konto = account
        local result = MySQL.Sync.fetchAll('SELECT * FROM players WHERE charinfo LIKE ?', {query})
        if result[1] ~= nil then
    ...
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if (Player.PlayerData.job.name == "lawyer" or Player.PlayerData.job.name == "realestate" or
                ...
                Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "ambulance" or Player.PlayerData.job.name == "cardealer") and
...
RegisterNetEvent('qb-phone:server:TransferMoney', function(iban, amount)
    ...
    if result[1] ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', src, "Dette kontonummer findes ikke!", "error")
    end
end)
...
QBCore.Commands.Add('bill', 'Send en regning til en spiller', {{name = 'id', help = 'Spiller ID'}, {name = 'amount', help = 'Bøde beløb'}}, false, function(source, args)
        ...
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    TriggerClientEvent('QBCore:Notify', source, 'Faktura blev sendt', 'success')
                    TriggerClientEvent('QBCore:Notify', billed.PlayerData.source, 'Ny faktura modtaget')
                else
                    TriggerClientEvent('QBCore:Notify', source, 'Skal være et gyldigt beløb over 0', 'error')
                end
            else
                TriggerClientEvent('QBCore:Notify', source, 'Du kan ikke fakturere dig selv', 'error')
            end
        else
