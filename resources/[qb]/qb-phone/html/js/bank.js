...
$(document).on('click', '.bank-app-account', function(e){
    ...
    QB.Phone.Notifications.Add("fas fa-university", "Bank", "Kontonummer. kopieret!", "#badc58", 1750);
});
...
QB.Phone.Functions.DoBankOpen = function() {
    ...
    $(".bank-app-account-balance").html(""+QB.Phone.Data.PlayerData.money.bank+" DKK");
    ...
    if (iban != "" && amount != "") {
            $.post('https://qb-phone/CanTransferMoney', JSON.stringify({
                ...
                if (data.TransferedMoney) {
                    ...
                    $(".bank-app-account-balance").html("" + (data.NewBalance).toFixed(0)+ " DKK");
                    $(".bank-app-account-balance").data('balance', (data.NewBalance).toFixed(0));
                    QB.Phone.Notifications.Add("fas fa-university", "Bank", "Du har overført "+amount+" DKK!", "#badc58", 1500);
                } else {
                    QB.Phone.Notifications.Add("fas fa-university", "Bank", "Du har ikke nok penge!", "#badc58", 1500);
                }
                QB.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
            });
    } else {
        QB.Phone.Notifications.Add("fas fa-university", "Bank", "Udfyld alle felter!", "#badc58", 1750);
    }
});
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('https://qb-phone/PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                QB.Phone.Notifications.Add("fas fa-university", "Bank", "Du har betalt "+InvoiceData.amount+" DKK!", "#badc58", 1500);
                var amountData = $(".bank-app-account-balance").data('balance');
...
QB.Phone.Functions.LoadBankInvoices = function(invoices) {
    if (invoices !== null) {
        ...
        $.each(invoices, function(i, invoice){
            var Elem = '<div class="bank-app-invoice" id="invoiceid-'+i+'"> <div class="bank-app-invoice-title">'+invoice.society+' <span style="font-size: 1vh; color: gray;">(Sender: '+invoice.sender+')</span></div> <div class="bank-app-invoice-amount">'+invoice.amount+' DKK</div> <div class="bank-app-invoice-buttons"> <i class="fas fa-check-circle pay-invoice"></i> <i class="fas fa-times-circle decline-invoice"></i> </div> </div>';
...
$(document).on('click', '.bank-app-my-contact', function(e){
    ...
    if (PressedContactData.iban !== "" && PressedContactData.iban !== undefined && PressedContactData.iban !== null) {
        ...
        QB.Phone.Notifications.Add("fas fa-university", "Bank", "Der er ingen bankkonto knyttet til dette nummer!", "#badc58", 2500);
    }
    QB.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});