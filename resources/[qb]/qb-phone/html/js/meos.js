...
$(document).on('click', '.person-search-result', function(e){
    ...
    var OpenElement = '<div class="person-search-result-name">Navn: '+ClickedPersonData.firstname+' '+ClickedPersonData.lastname+'</div> <div class="person-search-result-bsn">Borger ID: '+ClickedPersonData.citizenid+'</div> <div class="person-opensplit"></div> &nbsp; <div class="person-search-result-dob">Fødselsdato: '+ClickedPersonData.birthdate+'</div> <div class="person-search-result-number">Telefonnummer: '+ClickedPersonData.phone+'</div> <div class="person-search-result-nationality">Nationalitet: '+ClickedPersonData.nationality+'</div> <div class="person-search-result-gender">Køn: '+Gender+'</div> &nbsp; <div class="person-search-result-apartment"><span id="'+ClickedPersonId+'">Lejlighed: '+appartementData.label+'</span> <i class="fas fa-map-marker-alt appartment-adress-location" id="'+ClickedPersonId+'"></i></div> &nbsp; <div class="person-search-result-warned">Signaleret: '+IsWarrant+'</div> <div class="person-search-result-driverslicense">Kørekort: '+HasLicense+'</div>';
    ...
    if (OpenedPerson === null) {
        ...
        var PreviousElement = '<div class="person-search-result-name">Navn: '+PreviousPersonData.firstname+' '+PreviousPersonData.lastname+'</div> <div class="person-search-result-bsn">Borger ID: '+PreviousPersonData.citizenid+'</div>';
...
$(document).on('click', '.person-search-result-house', function(e){
    ...
    var OpenElement = '<div class="person-search-result-name">Ejer: '+ClickedHouseData.charinfo.firstname+' '+ClickedHouseData.charinfo.lastname+'</div><div class="person-search-result-bsn">Hus: '+ClickedHouseData.label+'</div> <div class="person-opensplit"></div> &nbsp; <div class="person-search-result-dob">Addresse: '+ClickedHouseData.label+' &nbsp; <i class="fas fa-map-marker-alt house-adress-location" id="'+ClickedHouseId+'"></i></div> <div class="person-search-result-number">Tier: '+ClickedHouseData.tier+'</div> <div class="person-search-result-nationality">Garage: ' + GarageLabel + '</div>';
    ...
    if (OpenedHouse === null) {
        ...
        var PreviousElement = '<div class="person-search-result-name">Ejer: '+PreviousPersonData.charinfo.firstname+' '+PreviousPersonData.charinfo.lastname+'</div> <div class="person-search-result-bsn">Hus: '+PreviousPersonData.label+'</div>';
        ...
        $.post('https://qb-phone/FetchSearchResults', JSON.stringify({
            ...
            if (result != null) {
                ...
                $.each(result, function (i, person) {
                    var PersonElement = '<div class="person-search-result" id="person-'+i+'"><div class="person-search-result-name">Navn: '+person.firstname+' '+person.lastname+'</div> <div class="person-search-result-bsn">Borger ID: '+person.citizenid+'</div> </div>';
        ...
        $.post('https://qb-phone/FetchVehicleResults', JSON.stringify({
            ...
            if (result != null) {
                ...
                $.each(result, function (i, vehicle) {
                    ...
                    var VehicleElement = '<div class="vehicle-search-result"> <div class="vehicle-search-result-name">'+vehicle.label+'</div> <div class="vehicle-search-result-plate">Nummerplade: '+vehicle.plate+'</div> <div class="vehicle-opensplit"></div> &nbsp; <div class="vehicle-search-result-owner">Ejer: '+vehicle.owner+'</div> &nbsp; <div class="vehicle-search-result-apk">MOT: '+APK+'</div> <div class="vehicle-search-result-warrant">Signaleret: '+Flagged+'</div> </div>'
...
AddPoliceAlert = function(data) {
    ...
    if (data.alert.coords != undefined && data.alert.coords != null) {
        AlertElement = '<div class="meos-alert" id="alert-'+randId+'"> <span class="meos-alert-new" style="margin-bottom: 1vh;">NEW</span> <p class="meos-alert-type">Alarm: '+data.alert.title+'</p> <p class="meos-alert-description">'+data.alert.description+'</p> <hr> <div class="meos-location-button">LOKATION</div> </div>';
    } else {
        AlertElement = '<div class="meos-alert" id="alert-'+randId+'"> <span class="meos-alert-new" style="margin-bottom: 1vh;">NEW</span> <p class="meos-alert-type">Alarm: '+data.alert.title+'</p> <p class="meos-alert-description">'+data.alert.description+'</p></div>';
    }
...
$(document).on('click', '.meos-recent-alert', function(e){
    ...
    if (alertData != undefined){
        ...
        QB.Phone.Notifications.Add("politie", "MDT", "Der er ingen tilgængelige alarmer.");
    }
});
