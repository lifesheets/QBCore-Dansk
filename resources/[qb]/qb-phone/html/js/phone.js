            ...
            if (status.IsOnline) {
                if (status.CanCall) {
                    if (!status.InCall) {
                        ...
                        QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "Du er allerede i et opkald!");
                    }
                } else {
                    QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "Denne person har travlt!");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "Denne person er ikke tilgængelig!");
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "Du kan ikke ringe til dig selv!");
        }
    });
});
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== QB.Phone.Data.PlayerData.charinfo.phone) {
        ...
        QB.Phone.Notifications.Add("fa fa-phone-alt", "Telefon", "Du kan ikke skrive til dig selv..", "default", 3500);
    }
});
...
$(document).on('click', '#edit-contact', function(e){
    ...
    $(".phone-edit-contact-header").text(ContactData.name+" Rediger")
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        ...
        QB.Phone.Notifications.Add("fas fa-exclamation-circle", "Rediger Kontakt", "Udfyld alle felter!");
    }
});
        ...
        if (cData.number !== QB.Phone.Data.PlayerData.charinfo.phone) {
            if (status.IsOnline) {
                if (status.CanCall) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "Denne person er i et opkald!");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "Denne person er ikke tilgængelig!");
            }
...
IncomingCallAlert = function(CallData, Canceled, AnonymousCall) {
    if (!Canceled) {
        if (!QB.Phone.Data.CallActive) {
            ...
            QB.Phone.Animations.TopSlideUp('.'+QB.Phone.Data.currentApplication+"-app", 400, -160);
            setTimeout(function(){
                var Label = "Du har et indgående opkald fra "+CallData.name
                if (AnonymousCall) {
                    Label = "Du bliver ringet op af en anonym person"
                }
...
QB.Phone.Functions.SetupCurrentCall = function(cData) {
    if (cData.InCall) {
        ...
        if (cData.CallType == "incoming") {
            $(".phone-currentcall-title").html("Indgående Opkald");
...
QB.Phone.Functions.SetupSuggestedContacts = function(Suggested) {
    ...
    if (AmountOfSuggestions > 0) {
        $(".amount-of-suggested-contacts").html(AmountOfSuggestions + " kontakter");
