...
function SetupCryptoData(Crypto) {
    ...
    $(".crypto-action-page-wallet").html("Pung: "+CryptoData.Portfolio+" Bits");
    ...
    if (CryptoData.History.length > 0) {
        ...
        $.each(CryptoData.History, function(i, change){
            ...
            var Element =   '<div class="cryptotab-course-block">' +
                                ...
                                '<span class="cryptotab-course-block-happening"><span style="font-size: 1.3vh;">'+change.PreviousWorth+' DKK</span> to <span style="font-size: 1.3vh;">'+change.NewWorth+' DKK</span>'+PercentageElement+'</span>' +
                            '</div>';
    ...
    $(".crypto-course").find('p').html(""+CryptoData.Worth+" DKK");
    $(".crypto-volume").find('p').html(""+Math.ceil(CryptoData.Portfolio * CryptoData.Worth)+" DKK");
}
...
$(document).on('click', '#buy-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (QB.Phone.Data.PlayerData.money.bank >= Price) {
            $.post('https://qb-phone/BuyCrypto', JSON.stringify({
                ...
                if (CryptoData !== false) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-university", "Bank", ""+Price+" DKK er blevet trukket fra din saldo!", "#badc58", 2500);
                } else {
                    QB.Phone.Notifications.Add("fas fa-chart-pie", "Krypto", "Du har ikke nok penge..", "#badc58", 1500);
                }
            });
        } else {
            QB.Phone.Notifications.Add("fas fa-chart-pie", "Krypto", "Du har ikke nok penge..", "#badc58", 1500);
        ...
        QB.Phone.Notifications.Add("fas fa-chart-pie", "Krypto", "Udfyld alle felter!", "#badc58", 1500);
    }
});
        ...
        if (CryptoData.Portfolio >= parseInt(Coins)) {
            $.post('https://qb-phone/SellCrypto', JSON.stringify({
                ...
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    QB.Phone.Data.PlayerData.money.bank = parseInt(QB.Phone.Data.PlayerData.money.bank) + parseInt(Price);
                    QB.Phone.Notifications.Add("fas fa-university", "Bank", ""+Price+" DKK er blevet tilføjet til din saldo!", "#badc58", 2500);
                } else {
                    QB.Phone.Notifications.Add("fas fa-chart-pie", "Krypto", "Du har ikke nok Bits..", "#badc58", 1500);
                }
            });
        } else {
            QB.Phone.Notifications.Add("fas fa-chart-pie", "Krypto", "Du har ikke nok Bits..", "#badc58", 1500);
            ...
            if (WalletId !== CryptoData.WalletId) {
                $.post('https://qb-phone/TransferCrypto', JSON.stringify({
                    ...
                    if (CryptoData == "notenough") {
                        ...
                        QB.Phone.Notifications.Add("fas fa-university", "Krypto", "dette Pung-ID findes ikke!", "#badc58", 2500);
                    } else {
                        UpdateCryptoData(CryptoData)
                        CloseCryptoPage()
                        QB.Phone.Notifications.Add("fas fa-university", "Krypto", "Du overførte "+Coins+",- til "+WalletId+"!", "#badc58", 2500);
                    }
                });
            } else {
                QB.Phone.Notifications.Add("fas fa-university", "Krypto", "Du kan ikke overføre til dig selv..", "#badc58", 2500);
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-chart-pie", "Krypto", "Du har ikke nok Bits..", "#badc58", 1500);
        }
...
$(".crypto-action-page-buy-crypto-input-coins").keyup(function(){
    ...
    $(".crypto-action-page-buy-crypto-input-money").html(MoneyAmount+" Kroner");
});
...
$(".crypto-action-page-sell-crypto-input-coins").keyup(function(){
    ...
    $(".crypto-action-page-sell-crypto-input-money").html(MoneyAmount+" Kroner");
});
