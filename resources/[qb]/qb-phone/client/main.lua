...
local function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Telefon",
                text = "Du har ikke et indgående opkald...",
                icon = "fas fa-phone",
                color = "#e84118",
            },
        })
...
RegisterCommand('phone', function()
    ...
    if not PhoneData.isOpen and LocalPlayer.state.isLoggedIn then
        if not PlayerData.metadata['ishandcuffed'] and not PlayerData.metadata['inlaststand'] and not PlayerData.metadata['isdead'] and not IsPauseMenuActive() then
            ...
            QBCore.Functions.Notify("Handlingen er ikke tilgængelig i øjeblikket..", "error")
        end
    end
end)
...
RegisterNUICallback('ClearRecentAlerts', function(data, cb)
    TriggerServerEvent('qb-phone:server:SetPhoneAlerts', "Telefon", 0)
...
RegisterNUICallback('track-vehicle', function(data, cb)
    ...
    if findVehFromPlateAndLocate(veh.plate) then
        QBCore.Functions.Notify("Dit køretøj blev markeret", "success")
    else
        QBCore.Functions.Notify("Dette køretøj kan ikke lokaliseres", "error")
    end
end)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    QBCore.Functions.Notify("GPS er indstillet to " .. data.HouseData.HouseData.adress .. "!", "success")
end)
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
		...
		SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { timeout=3000, title = "Bank", text = "Kontoen findes ikke!", icon = "fas fa-university", color = "#ff0000", }, })
...
RegisterCommand('ping', function(source, args)
    local PlayerData = QBCore.Functions.GetPlayerData()
    if not args[1] then
        QBCore.Functions.Notify("Du skal indtaste et ID", "error")
    else
...
RegisterNetEvent('qb-phone:client:TransferMoney', function(amount, newmoney)
    ...
    SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "Bank", text = ""..amount.." DKK er blevet tilføjet til din konto!", icon = "fas fa-university", color = "#8c7ae6", }, })
...
RegisterNetEvent('qb-phone:client:UpdateTweets', function(src, Tweets, NewTweetData, delete)
    PhoneData.Tweets = Tweets
    local MyPlayerId = PhoneData.PlayerData.source
    if not delete then -- Nyt Tweet
        if src ~= MyPlayerId then
            SendNUIMessage({
                action = "PhoneNotification",
                PhoneNotify = {
                    title = "Nyt Tweet (@"..NewTweetData.firstName.." "..NewTweetData.lastName..")",
                    text = "Et nyt Tweet er blevet posted.",
                    icon = "fab fa-twitter",
                    color = "#1DA1F2",
                },
            })
        ...
        if src == MyPlayerId then
            SendNUIMessage({
                ...
                PhoneNotify = {
                    title = "Twitter",
                    text = "Tweetet er blevet slettet!",
                    icon = "fab fa-twitter",
                    color = "#1DA1F2",
                    timeout = 1000,
                },
...
RegisterNetEvent('qb-phone:client:UpdateAdverts', function(Adverts, LastAd)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Annoncer",
            text = "En ny annonce er blevet sendt af "..LastAd,
            icon = "fas fa-ad",
            color = "#ff8f1a",
            timeout = 2500,
        },
...
RegisterNetEvent('qb-phone:client:BillingEmail', function(data, paid, name)
    if paid then
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = 'Faktureringsafdeling',
            subject = 'Faktura Betalt',
            message = 'Faktura Er Betalt Fra '..name..' På '..data.amount..' DKK',
        })
    else
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = 'Faktureringsafdeling',
            subject = 'Faktura afvist',
            message = 'Faktura Er Blevet Afvist Af '..name..' På '..data.amount..' DKK',
        })
    end
end)
...
RegisterNetEvent('qb-phone:client:RemoveBankMoney', function(amount)
    if amount > 0 then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Bank",
                text = ""..amount.." DKK er blevet fjernet fra din saldo!",
                icon = "fas fa-university",
                color = "#ff002f",
                timeout = 3500,
            },
...
RegisterNetEvent('qb-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    Config.PhoneApplications["phone"].Alerts = Config.PhoneApplications["phone"].Alerts + 1
    TriggerServerEvent('qb-phone:server:SetPhoneAlerts', "Telefon", Config.PhoneApplications["phone"].Alerts)
end)
...
RegisterNetEvent('qb-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
    ...
    SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "Du er blevet nævnt i et tweet!", text = TweetMessage.message, icon = "fab fa-twitter", color = "#1DA1F2", }, })
