...
function lockpickDone(success)
    ...
    if success then
        ...
        if math.random(1, 100) <= 40 and IsWearingHandshoes() then
            TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
            QBCore.Functions.Notify("Du rev ​​din handske..")
        end
...
function GrabItem(spot)
    ...
    QBCore.Functions.Progressbar("grab_ifruititem", "Afbryd element", 10000, false, true, {
        ...
        TriggerServerEvent('qb-ifruitstore:server:PoliceAlertMessage', 'Folk forsøger at stjæle varer fra iFruit butikken', pos, true)
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "anim@gangops@facility@servers@", "hotwire", 1.0)
        TriggerServerEvent('qb-ifruitstore:server:setSpotState', "isBusy", false, spot)
...
RegisterNUICallback('thermitefailed', function()
    ...
    TriggerServerEvent('qb-ifruitstore:server:PoliceAlertMessage', 'Folk forsøger at stjæle varer fra iFruit butikken', coords, true)
end)
...
RegisterNetEvent('qb-ifruitstore:client:PoliceAlertMessage', function(msg, coords, blip)
    if blip then
        ...
        TriggerEvent("chatMessage", "112-Rapport", "error", msg)
        local transG = 100
        ...
        AddTextComponentString("112 - Mistænkelig situation i iFruit butikken")
...
RegisterNetEvent('qb-ifruitstore:client:robberyCall', function(streetLabel, coords)
    if PlayerJob.name == "police" then
        ...
        TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
            timeOut = 10000,
            alertTitle = "iFruit Butik Røveriforsøg",
            coords = {
                x = coords.x,
                y = coords.y,
                z = coords.z,
        ...
        BeginTextCommandSetBlipName('STRING')
        AddTextComponentString("10-90: iFruit Butik Røveri")
        EndTextCommandSetBlipName(blip)
        while transG ~= 0 do
            Wait(180 * 4)
            transG = transG - 1
                ...
                if dist < 1.0 then
                    ...
                    if dist < 0.6 then
                        ...
                        if not Config.Locations["takeables"][spot].isBusy and not Config.Locations["takeables"][spot].isDone then
                            DrawText3Ds(Config.Locations["takeables"][spot].x, Config.Locations["takeables"][spot].y,Config.Locations["takeables"][spot].z, '~g~E~w~ For at tage varen')
                            if IsControlJustPressed(0, 38) then
                                if CurrentCops >= 0 then
                                    if Config.Locations["thermite"].isDone then
                                        ...
                                        QBCore.Functions.Notify("Sikkerheden er stadig aktiv..", "error")
                                    end
                                else
                                    QBCore.Functions.Notify("Ikke nok Politi", "error")
                                end
