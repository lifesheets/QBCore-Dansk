...
RegisterCommand('announce', function(source, args, rawCommand)
	local msg = rawCommand:sub(10)
        TriggerClientEvent('chat:addMessage', -1, {
        template = '<div class="chat-message server"><b><strong>NYHED:</strong></b> {0}</div>',
        args = { msg },
        color = { 255, 255 , 255}
    })
end, false)
...
QBCore.Commands.Add('report', Lang:t("info.admin_report"), {{name='message', help='ID', help='Message'}}, true, function(source, args)
    local src = source
    local msg = table.concat(args, ' ')
