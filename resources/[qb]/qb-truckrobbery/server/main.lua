...
if ActiveMission == 0 then
	...
	TriggerClientEvent('QBCore:Notify', _source, "Du skal have "..ActivationCost..",- i din bank for at acceptere missionen")
	else
	...
	if copsOnDuty >= ActivePolice then
		...
		TriggerClientEvent('QBCore:Notify', _source, 'Der skal være mindst '..ActivePolice.. ' Politi for at aktivere missionen.')
    end
	end
else
TriggerClientEvent('QBCore:Notify', _source, 'Nogen udfører allerede denne mission')
end
end)
...
AddEventHandler('qb-armoredtruckheist:server:callCops', function(streetLabel, coords)
    ...
    local msg = "Alarmen er blevet aktiveret fra en "..place.. " ved " ..streetLabel
...
AddEventHandler('AttackTransport:graczZrobilnapad', function(moneyCalc)
	...
	TriggerClientEvent('QBCore:Notify', _source, 'Du tog '..bags..' poser med kontanter fra lastbilen')
