...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
            ...
            if dist <= 1.0 then
				DrawText3D(MissionMarker.x, MissionMarker.y, MissionMarker.z, "~g~[E]~b~ For at acceptere missioner")
...
AddEventHandler('AttackTransport:InfoForLspd', function(x, y, z)
	if PlayerJob ~= nil and PlayerJob.name == 'police' then
		...
		if PoliceBlip == 0 then
			...
			AddTextComponentString('Overfald på transport af kontanter')
    ...
    if PlayerJob.name == "police" then
        local store = "Armored Truck"
            ...
            TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
                timeOut = 10000,
                alertTitle = "Forsøg på røveri af pansret lastbil",
                coords = {
                    x = coords.x,
                    y = coords.y,
                    z = coords.z,
        ...
        BeginTextCommandSetBlipName('STRING')
        AddTextComponentString("10-90: Pansret lastbilrøveri")
        EndTextCommandSetBlipName(blip)
        while transG ~= 0 do
            Wait(180 * 4)
            transG = transG - 1
...
function MissionNotification()
	...
	sender = "Boss",
	subject = "Nyt Mål",
	message = "Så du er interesseret i at tjene nogle penge? godt... hent dig en pistol og få det til at ske... sender dig lokationen nu.",
	})
	Citizen.Wait(3000)
end
---
		...
		if MissionStart == 1 then
			...
			if dist <= 7 and BlownUp == 0 and PlayerJob.name ~= 'police' then
				if BlowBackdoor == 0 then
					hintToDisplay('Tryk [G] for at sprænge bagdøren og tage pengene')
					BlowBackdoor = 1
				end
				if IsControlPressed(0, 47) and GuardsDead == 1 then
...
if IsVehicleStopped(transport) then
	if IsVehicleSeatFree(transport, -1) and IsVehicleSeatFree(transport, 0) and IsVehicleSeatFree(transport, 1) and GuardsDead == 1 then
		if not IsEntityInWater(PlayerPedId()) then
			...
			QBCore.Functions.Notify('Ladningen vil blive detoneret om '..TimeToBlow / 1000 ..' sekunder.', "error")
			...
			BlownUp = 1
			lootable = 1
			QBCore.Functions.Notify('Du kan begynde at samle penge.', "success")
			RemoveBlip(TruckBlip)
		else
			QBCore.Functions.Notify('Kom op af vandet', "error")
		end
	else
		QBCore.Functions.Notify('Køretøjet skal være tomt for at kunne placere lasten', "error")
	end
else
	QBCore.Functions.Notify('Du kan ikke røve et køretøj, der er i bevægelse.', "error")
end
end
		...
		if lootable == 1 then
			...
			if dist <= 4.5 then
				if PickupMoney == 0 then
					hintToDisplay('Tryk [E] for at tage pengene')
					PickupMoney = 1
				end
				if IsControlJustPressed(0, 38) then
				lootable = 0
...
function TakingMoney()
	...
	FreezeEntityPosition(PlayerPedId(), true)
	QBCore.Functions.Notify('Du pakker kontanter i en pose', "success")
	local _time = GetGameTimer()
	while GetGameTimer() - _time < 20000 do
		if IsControlPressed(0, 47) then
			break
		end
		hintToDisplay('Hold [G] for at annullere')
		Citizen.Wait(1)
	end
