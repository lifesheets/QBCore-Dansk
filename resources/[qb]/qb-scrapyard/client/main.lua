...
CreateThread(function()
	for id, scrapyard in pairs(Config.Locations) do
        ...
        AddTextComponentSubstringPlayerName("Losseplads")
				...
				if IsPedInAnyVehicle(PlayerPedId()) then
					...
					if vehicle ~= 0 and vehicle ~= nil then
						...
						if #(pos - vector3(vehpos.x, vehpos.y, vehpos.z)) < 2.5 and not isBusy then
							DrawText3Ds(vehpos.x, vehpos.y, vehpos.z, "~g~E~w~ - Skil køretøj ad")
							if IsControlJustReleased(0, 38) then
								if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
									if IsVehicleValid(GetEntityModel(vehicle)) then
										...
										QBCore.Functions.TriggerCallback('qb-scrapyard:checkOwnerVehicle',function(retval)
											if retval then
												...
												QBCore.Functions.Notify("Du kan ikke smadre et køretøj, der er ejet.", "error")
											end
										end,vehiclePlate)
									else
									...
									QBCore.Functions.Notify("Du er ikke chaufføren", "error")
								end
							end
						end
					end
	...
	if Config.CurrentVehicles ~= nil and next(Config.CurrentVehicles) ~= nil then
		...
		SetTimeout(math.random(15000, 20000), function()
			...
			TriggerServerEvent('qb-phone:server:sendNewMail', {
				...
				subject = "Køretøjs Liste",
				message = "Du kan kun rive et antal køretøjer ned.<br />Du kan beholde alt, hvad du river ned for dig selv, så længe du ikke generer mig.<br /><br /><strong>Køretøjs Liste:</strong><br />".. vehicleList,
				button = {}
			})
		end)
	else
...
function ScrapVehicle(vehicle)
	...
	QBCore.Functions.Progressbar("scrap_vehicle", "Nedriv køretøj", scrapTime, false, true, {
