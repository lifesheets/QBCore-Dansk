    ...
    if not NitrousActivated then
        if IsInVehicle and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
            if GetPedInVehicleSeat(veh, -1) == ped then
                QBCore.Functions.Progressbar("use_nos", "Forbinder Nitro...", 1000, false, true, {
                ...
                QBCore.Functions.Notify("Det kan du ikke gøre fra dette sæde!", "error")
            end
        else
            QBCore.Functions.Notify('Du er ikke i en bil', 'error')
        end
    else
        QBCore.Functions.Notify('Du har allerede nitro aktivt', 'error')
    end
end)
