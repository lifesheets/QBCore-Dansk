...
RegisterNetEvent('inventory:client:CraftItems', function(itemName, itemCosts, amount, toSlot, points)
    ...
    isCrafting = true
    QBCore.Functions.Progressbar("repair_vehicle", "Samler..", (math.random(2000, 5000) * amount), false, true, {
    ...
    if not isCrafting and not inInventory then
        if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] and not IsPauseMenuActive() then
            ...
            elseif CurrentDrop then
                TriggerServerEvent("inventory:server:OpenInventory", "dropped", CurrentDrop)
...
RegisterKeyMapping('inventory', 'Åben Inventory', 'keyboard', 'TAB')
...
RegisterNUICallback("CloseInventory", function()
    ...
    elseif CurrentStash ~= nil then
        ...
        TriggerServerEvent("inventory:server:SaveInventory", "dropped", CurrentDrop)
        CurrentDrop = nil
    end
    SetNuiFocus(false, false)
    inInventory = false
...
RegisterNUICallback("GiveItem", function(data)
    ...
    if player ~= -1 and distance < 3 then
        if (data.inventory == 'player') then
            ...
            QBCore.Functions.Notify("Du ejer ikke denne ting!", "error")
        end
    else
        ...
        if LocalPlayer.state['isLoggedIn'] then
            ...
            if distance < 10 then
                if distance < 1.5 then
                    sleep = 0
                    DrawText3Ds(Config.AttachmentCraftingLocation.x, Config.AttachmentCraftingLocation.y, Config.AttachmentCraftingLocation.z, "~g~E~w~ - Saml")
