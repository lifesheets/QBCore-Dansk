...
RegisterNetEvent('inventory:server:OpenInventory', function(name, id, other)
	...
	if not ply.state.inv_busy then
		if name ~= nil and id ~= nil then
			...
			elseif name == "trunk" then
				...
				if (Trunks[id] ~= nil and Trunks[id].isOpen) or (QBCore.Shared.SplitStr(id, "PLZI")[2] ~= nil and Player.PlayerData.job.name ~= "police") then
					secondInv.name = "none-inv"
					secondInv.label = "Bagagerum-Ingen"
					secondInv.maxweight = other.maxweight ~= nil and other.maxweight or 60000
					secondInv.inventory = {}
					secondInv.slots = 0
				else
			...
			elseif name == "glovebox" then
				...
				secondInv.label = "Handskerum-"..id
				secondInv.maxweight = 10000
				secondInv.inventory = {}
				secondInv.slots = 5
				if Gloveboxes[id] ~= nil and Gloveboxes[id].isOpen then
					secondInv.name = "none-inv"
					secondInv.label = "Handskerum-Ingen"
					secondInv.maxweight = 10000
					secondInv.inventory = {}
					secondInv.slots = 0
				else
			...
			elseif name == "otherplayer" then
				...
				if Drops[id] ~= nil and not Drops[id].isOpen then
					secondInv.name = id
					secondInv.label = "Spiller-"..tostring(id)
		...
		TriggerClientEvent('QBCore:Notify', src, 'Du kan ikke gøre dette lige nu', 'error')
	end
end)
...
QBCore.Commands.Add("rob", "Røv en Spiller", {}, false, function(source, args)
		...
		if amount > 0 then
			if itemData ~= nil then
				...
				if Player.Functions.AddItem(itemData["name"], amount, false, info) then
					TriggerClientEvent('QBCore:Notify', source, "Du Har Givet " ..GetPlayerName(tonumber(args[1])).." "..amount.." "..itemData["name"].. "", "success")
				else
					TriggerClientEvent('QBCore:Notify', source,  "Kan Ikke Give Tingen!", "error")
				end
			else
				TriggerClientEvent('QBCore:Notify', source,  "Tingen Eksisterer Ikke", "error")
			end
		else
			TriggerClientEvent('QBCore:Notify', source,  "Ugyldigt Beløb", "error")
		end
	else
		TriggerClientEvent('QBCore:Notify', source,  "Spiller Er Ikke Online", "error")
	end
end, "admin")
	...
	for k, v in pairs(QBCore.Functions.GetPlayers()) do
		...
		if dist < 3.0 then
			TriggerClientEvent('chat:addMessage', v,  {
					--					template = '<div class="chat-message advert"><div class="chat-message-body"><strong>{0}:</strong><br><br> <strong>Navn:</strong> {1} <br><strong>Efternavn:</strong> {2} <br><strong>Fødselsdato:</strong> {3} <br><strong>Type:</strong> {4}</div></div>',
					template = '<div class="chat-message advert"><div class="chat-message-body"><strong>{0}:</strong><br><br> <strong>Navn:</strong> {1} <br><strong>Efternavn:</strong> {2} <br><strong>Fødselsdato:</strong> {3}</div></div>',
					args = {
						"Kørekort",
						item.info.firstname,
						item.info.lastname,
...
QBCore.Functions.CreateUseableItem("id_card", function(source, item)
	for k, v in pairs(QBCore.Functions.GetPlayers()) do
		...
		if dist < 3.0 then
			...
			TriggerClientEvent('chat:addMessage', v,  {
					template = '<div class="chat-message advert"><div class="chat-message-body"><strong>{0}:</strong><br><br> <strong>Borger ID:</strong> {1} <br><strong>Navn:</strong> {2} <br><strong>Efternavn:</strong> {3} <br><strong>Fødselsdato:</strong> {4} <br><strong>Køn:</strong> {5} <br><strong>Nationalitet:</strong> {6}</div></div>',
					args = {
						"ID Kort",
						item.info.citizenid,
						item.info.firstname,
