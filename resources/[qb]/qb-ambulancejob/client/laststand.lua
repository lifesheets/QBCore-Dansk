...
function SetLaststand(bool, spawn)
	...
	TriggerServerEvent('hospital:server:ambulanceAlert', 'Civil Nede')
        ...
        CreateThread(function()
            while InLaststand do
                ...
                elseif LaststandTime - 1 <= 0 then
                    QBCore.Functions.Notify("Du har blødt ud..", "error")
                    SetLaststand(false)
...
RegisterNetEvent('hospital:client:UseFirstAid', function()
    if not isEscorting then
        ...
        QBCore.Functions.Notify('Handling umulig!', 'error')
    end
end)
