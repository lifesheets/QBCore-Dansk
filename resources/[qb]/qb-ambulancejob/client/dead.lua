...
--[[                loadAnimDict("veh@low@front_ps@idle_duck")
            ...
            TriggerServerEvent('hospital:server:ambulanceAlert', 'Borger Døde')
        end
    end
end
	...
	while true do
		...
		if NetworkIsPlayerActive(player) then
            ...
            elseif IsEntityDead(playerPed) and InLaststand and not isDead then
                ...
                local killerName = killerId ~= -1 and GetPlayerName(killerId) .. " " .. "("..GetPlayerServerId(killerId)..")" or "Sig selv eller en NPC"
                ...
                TriggerServerEvent("qb-log:server:CreateLog", "death", GetPlayerName(-1) .. " ("..GetPlayerServerId(player)..") er død", "red", "**".. killerName .. "** har dræbt ".. GetPlayerName(player) .." med en **".. weaponLabel .. "** (" .. weaponName .. ")")
		...
		if isDead or InLaststand then
            ...
            if isDead then
                if not isInHospitalBed then
                    if deathTime > 0 then
                        ...
                        DrawTxt(0.865, 1.44, 1.0, 1.0, 0.6, "~w~ HOLD ~r~[E] ("..hold..")~w~ FOR AT GENOPSTÅ ~r~("..Config.BillCost.." DKK)~w~", 255, 255, 255, 255)
                    end
                end
            ...
            elseif InLaststand then
                ...
                if LaststandTime > Laststand.MinimumRevive then
                    DrawTxt(0.900, 1.44, 1.0, 1.0, 0.6, "DU BLØDER UD OM: ~r~" .. math.ceil(LaststandTime) .. "~w~ SEKUNDER", 255, 255, 255, 255)
                else
                    DrawTxt(0.900, 1.44, 1.0, 1.0, 0.6, "DU BLØDER UD OM: ~r~" .. math.ceil(LaststandTime) .. "~w~ SEKUNDER", 255, 255, 255, 255)
                end
