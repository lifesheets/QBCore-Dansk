    ...
    QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
        if hasItem then
            ...
            if player ~= -1 and distance < 5.0 then
                ...
                QBCore.Functions.Progressbar("hospital_revive", "Genopliver person..", 5000, false, true, {
                    ...
                    QBCore.Functions.Notify("Du genoplivede personen!")
            ...
            QBCore.Functions.Notify("Du Mangler Et Medkit", "error")
        end
    end, 'firstaid')
end)
    ...
    QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
        if hasItem then
            ...
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                isHealingPerson = true
                QBCore.Functions.Progressbar("hospital_healwounds", "Heler Sår..", 5000, false, true, {
                    disableMovement = false,
                    disableCarMovement = false,
                    disableMouse = false,
                    disableCombat = true,
                    ...
                    isHealingPerson = false
                    StopAnimTask(PlayerPedId(), healAnimDict, "exit", 1.0)
                    QBCore.Functions.Notify("Du hjalp personen!")
            ...
            QBCore.Functions.Notify("Du Mangler En Bandage", "error")
        end
    end, 'bandage')
end)
            ...
            for k, v in pairs(Config.Locations["stash"]) do
                if #(pos - v) < 4.5 then
                    if onDuty then
                        sleep = 5
                        if #(pos - v) < 1.5 then
                            DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Personlige stash")
                        ...
                        elseif #(pos - v) < 2.5 then
                            DrawText3D(v.x, v.y, v.z, "Personlige stash")
                        end
                    end
                end
            end
    ...
    while true do
        ...
        if LocalPlayer.state.isLoggedIn then
                ...
                for k, v in pairs(Config.Locations["duty"]) do
                    ...
                    if dist < 5 then
                        ...
                        elseif dist < 4.5 then
                            DrawText3D(v.x, v.y, v.z, "På/Af Job")
                        end
                    end
                end
                ...
                for k, v in pairs(Config.Locations["armory"]) do
                    ...
                    if dist < 4.5 then
                        if onDuty then
                            if dist < 1.5 then
                                sleep = 0
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Butik")
                                if IsControlJustReleased(0, 38) then
                                    TriggerServerEvent("inventory:server:OpenInventory", "shop", "hospital", Config.Items)
                                end
                            elseif dist < 2.5 then
                                DrawText3D(v.x, v.y, v.z, "Butik")
                            end
                        end
                    end
                end
                    ...
                    if dist < 4.5 then
                        ...
                        if dist < 1.5 then
                            if IsPedInAnyVehicle(ped, false) then
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Parker køretøj")
                            else
                ...
                for k, v in pairs(Config.Locations["helicopter"]) do
                    ...
                    if dist < 7.5 then
                        if onDuty then
                            ...
                            if dist < 1.5 then
                                if IsPedInAnyVehicle(ped, false) then
                                    DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Parker helikopter")
                                else
                                    DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Tag en helikopter")
                                end
                                if IsControlJustReleased(0, 38) then
                                    if IsPedInAnyVehicle(ped, false) then
                                        QBCore.Functions.DeleteVehicle(GetVehiclePedIsIn(ped))
            ...
            for k, v in pairs(Config.Locations["main"]) do
                ...
                if dist < 1.5 then
                    sleep = 5
                    DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Tag elevatoren til taget")
                    if IsControlJustReleased(0, 38) then
                        DoScreenFadeOut(500)
            ...
            for k, v in pairs(Config.Locations["roof"]) do
                ...
                if dist < 1.5 then
                    sleep = 5
                    DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Tag elevatoren ned")
                    if IsControlJustReleased(0, 38) then
                        DoScreenFadeOut(500)
                        while not IsScreenFadedOut() do
                            Wait(10)
