	...
	for k, v in pairs(Config.Locations["beds"]) do
		if not v.taken then
			...
			if Config.WipeInventoryOnRespawn then
				...
				TriggerClientEvent('QBCore:Notify', src, 'Alle dine ejendele er blevet taget..', 'error')
			end
			Player.Functions.RemoveMoney("bank", Config.BillCost, "respawned-at-hospital")
			TriggerEvent('qb-bossmenu:server:addAccountMoney', "ambulance", Config.BillCost)
			TriggerClientEvent('hospital:client:SendBillEmail', src, Config.BillCost)
	...
	if Patient then
		if oldMan then
			if Player.Functions.RemoveMoney("cash", 5000, "revived-player") then
				...
				TriggerClientEvent('QBCore:Notify', src, "Du har ikke penge nok på dig..", "error")
			end
		else
			Player.Functions.RemoveItem('firstaid', 1)
			TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['firstaid'], "remove")
...
RegisterNetEvent('hospital:server:SendDoctorAlert', function()
    ...
    for k,v in pairs(players) do
        if v.PlayerData.job.name == 'ambulance' and v.PlayerData.job.onduty then
			TriggerClientEvent('QBCore:Notify', v.PlayerData.source, 'Der er brug for en læge på Pillbox Hospital', 'ambulance')
		end
	end
end)
...
QBCore.Commands.Add('112e', 'EMS Report', {{name='message', help='Besked Der Skal Sendes'}}, false, function(source, args)
	local src = source
	if args[1] then message = table.concat(args, " ") else message = 'Borger Opkald' end
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    local players = QBCore.Functions.GetQBPlayers()
    for k,v in pairs(players) do
...
QBCore.Commands.Add("status", "Tjek En Borgers Helbred", {}, false, function(source, args)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	if Player.PlayerData.job.name == "ambulance" then
