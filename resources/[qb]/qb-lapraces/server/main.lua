...
RegisterNetEvent('qb-lapraces:server:FinishPlayer', function(RaceData, TotalTime, TotalLaps, BestLap)
    ...
    if Races[RaceData.RaceId].Records ~= nil and next(Races[RaceData.RaceId].Records) ~= nil then
        if BLap < Races[RaceData.RaceId].Records.Time then
            ...
            TriggerClientEvent('qb-phone:client:RaceNotify', src, 'Du har vundet verdensrekorden fra ' .. RaceData.RaceName ..
                ' med en tid på: ' .. SecondsToClock(BLap) .. '!')
        end
    else
        Races[RaceData.RaceId].Records = {
            Time = BLap,
...
RegisterNetEvent('qb-lapraces:server:CreateLapRace', function(RaceName)
    ...
    if IsWhitelisted(Player.PlayerData.citizenid) then
        if IsNameAvailable(RaceName) then
            ...
            TriggerClientEvent('QBCore:Notify', source, 'Der er allerede et ræs med dette navn.', 'error')
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'Du har ikke fået tilladelse til at lave ræs.', 'error')
    end
end)
...
RegisterNetEvent('qb-lapraces:server:JoinRace', function(RaceData)
    ...
    if CurrentRace ~= nil then
        ...
        if (AmountOfRacers - 1) == 0 then
            ...
            TriggerClientEvent('QBCore:Notify', src, 'Du var den eneste i ræset, ræset et sluttet', 'error')
...
RegisterNetEvent('qb-lapraces:server:LeaveRace', function(RaceData)
    ...
    if (AmountOfRacers - 1) == 0 then
        ...
        table.remove(AvailableRaces, AvailableKey)
        TriggerClientEvent('QBCore:Notify', src, 'Du var den eneste i ræset. Ræset er sluttet.', 'error')
    ...
    if Races[RaceId] ~= nil then
        if not Races[RaceId].Waiting then
            if not Races[RaceId].Started then
                ...
                TriggerClientEvent('QBCore:Notify', source, 'Ræset kører allerede', 'error')
            end
        else
            TriggerClientEvent('QBCore:Notify', source, 'Ræset kører allerede', 'error')
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'Dette ræs eksisterer ikke :(', 'error')
    end
end)
...
RegisterNetEvent('qb-lapraces:server:StartRace', function(RaceId)
    ...
    if RaceId ~= nil then
        if AvailableRaces[AvailableKey].SetupCitizenId == MyPlayer.PlayerData.citizenid then
            ...
            TriggerClientEvent('QBCore:Notify', src, 'Du er ikke skaberen af ​​løbet..', 'error')
        end
    else
        TriggerClientEvent('QBCore:Notify', src, 'Du er ikke i et ræs..', 'error')
    end
end)
...
QBCore.Commands.Add("cancelrace", "Aflys igangværende ræs..", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    ...
    if IsWhitelisted(Player.PlayerData.citizenid) then
        ...
        if RaceName ~= nil then
            ...
            if Races[RaceId].Started then
                ...
                TriggerClientEvent('QBCore:Notify', source, 'Dette ræs er ikke startet endnu.', 'error')
            end
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'Du har ikke fået tilladelse til at gøre dette.', 'error')
    end
end)
...
QBCore.Commands.Add("togglesetup", "Turn on / off racing setup", {}, false, function(source, args)
    ...
    if IsWhitelisted(Player.PlayerData.citizenid) then
        ...
        if not Config.RaceSetupAllowed then
            TriggerClientEvent('QBCore:Notify', source, 'Der kan ikke oprettes flere ræs!', 'error')
        else
            TriggerClientEvent('QBCore:Notify', source, 'RÆs kan oprettes igen!', 'success')
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'Du har ikke fået tilladelse til at gøre dette.', 'error')
    end
