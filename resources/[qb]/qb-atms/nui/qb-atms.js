...
function ATMStatement(data) {
    ...
    if(data !== undefined) {
        ...
        $("#currentStatementContentsATM").append('<tr class="statement"><td><small>' + statement.date + '</small></td><td><small>' + statement.type + '</small></td><td class="text-center text-danger"><small>' + withdraw + ' DKK</small></td><td class="text-center text-success"><small>' + deposit + ' DKK</small></td><td class="text-center"><small>' + balance + '</small></td></tr>');
        ...
        if ($('#pinContainer').is(':visible')) {
            if (key.which >= 48 && key.which <= 57) { // 48 - 57 (0-9 keys)
                ...
                if (currentVal == clientPin) {
                    ...
                    $('#errorMsg').html('Venligst indtast koden til dit kreditkort.');
                    $('#pinCode').val('');
                    ...
                    $('#errorMsg').html('Du har indtastet en forkert pinkode, prøv venligst igen.')
                    setTimeout(function () {
                        $('#errorMsg').addClass('alert-info').removeClass('alert-danger');
                        $('#errorMsg').html('Venligst indtast koden til dit kreditkort.');
                    }, 5000);
