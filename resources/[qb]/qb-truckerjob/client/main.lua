...
local function getNewLocation()
    ...
    if location ~= 0 then
        ...
        QBCore.Functions.Notify("Du gik i alle butikker .. Tid til din lønseddel!")
...
function RunWorkThread()
    if not ThreadAlreadyRan then
        ...
        CreateThread(function()
            ...
            SetBlipSprite(TruckerBlip, 0)
            SetBlipDisplay(TruckerBlip, 0)
                ...
                if #(pos - vehicleCoords) <= 5.0 then
                    ...
                    if #(pos - vehicleCoords) <= 1.5 then
                        if IsPedInAnyVehicle(PlayerPedId(), false) then
                            DrawText3D(x,y,z, "~g~E~w~ - Parker Køretøj")
                        else
                        ...
                        if IsControlJustReleased(0, 38) then
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then
                                    ...
                                    QBCore.Functions.Notify('Du skal være chauffør for at gøre dette..')
                                end
                            end
                        end
                    end
                    ...
                    if #(pos - mainCoords) < 1.5 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            if JobsDone > 0 then
                                ...
                                QBCore.Functions.Notify("Du har ikke gjort noget arbejde endnu..", "error")
                            end
                        end
                        ...
                        if not hasBox then
                            ...
                            if isTruckerVehicle(vehicle) and CurrentPlate == QBCore.Functions.GetPlate(vehicle) then
                                ...
                                if #(pos - vector3(trunkpos.x, trunkpos.y, trunkpos.z)) < 1.5 and not isWorking then
                                    DrawText3D(trunkpos.x, trunkpos.y, trunkpos.z, "~g~E~w~ - Afhent Produkter")
                                    if IsControlJustReleased(0, 38) then
                                        isWorking = true
                                        QBCore.Functions.Progressbar("work_carrybox", "Tager En Kasse Med Produkter", 2000, false, true, {
                                    ...
                                    DrawText3D(trunkpos.x, trunkpos.y, trunkpos.z, "Afhent Produkter")
                                end
                            end
                        elseif hasBox then
                            if #(pos - vector3(CurrentLocation.x, CurrentLocation.y, CurrentLocation.z)) < 1.5 and not isWorking then
                                DrawText3D(CurrentLocation.x, CurrentLocation.y, CurrentLocation.z, "~g~E~w~ - Aflever Produkter")
                                if IsControlJustReleased(0, 38) then
                                    isWorking = true
                                    TriggerEvent('animations:client:EmoteCommandStart', {"c"})
                                    Wait(500)
                                    TriggerEvent('animations:client:EmoteCommandStart', {"bumbin"})
                                    QBCore.Functions.Progressbar("work_dropbox", "Leverer kasse med produkter", 2000, false, true, {
                                        disableMovement = true,
                                        disableCarMovement = true,
                                        disableMouse = false,
                                        disableCombat = true,
                                        ...
                                        if currentCount == CurrentLocation.dropcount then
                                            ...
                                            QBCore.Functions.Notify("Du har leveret alle produkter, kør til det næste punkt")
                                ...
                                DrawText3D(CurrentLocation.x, CurrentLocation.y, CurrentLocation.z, "Aflever Produkter")
                            end
                        end
                    end
                end
