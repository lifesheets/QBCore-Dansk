...
RegisterNetEvent('qb-trucker:server:DoBail', function(bool, vehInfo)
    ...
    if bool then
        if Player.PlayerData.money.cash >= Config.BailPrice then
            ...
            TriggerClientEvent('QBCore:Notify', src, '250,- Depositum Betalt Med Kontanter', 'success')
        ...
        elseif Player.PlayerData.money.bank >= Config.BailPrice then
            ...
            TriggerClientEvent('QBCore:Notify', src, '250,- Depositum Betalt Med Bank', 'success')
            TriggerClientEvent('qb-trucker:client:SpawnVehicle', src, vehInfo)
        else
            TriggerClientEvent('QBCore:Notify', src, '250,- Depositum påkrævet', 'error')
        end
    else
        if Bail[Player.PlayerData.citizenid] ~= nil then
            Player.Functions.AddMoney('cash', Bail[Player.PlayerData.citizenid], "trucker-bail-paid")
            Bail[Player.PlayerData.citizenid] = nil
            TriggerClientEvent('QBCore:Notify', src, '250,- Depositum Refunderes Til Kontanter', 'success')
        end
    end
end)
...
RegisterNetEvent('qb-trucker:server:01101110', function(drops)
    ...
    local bonus = 0
    local DropPrice = math.random(1000, 1200)
