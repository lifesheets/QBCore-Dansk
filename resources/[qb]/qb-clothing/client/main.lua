...
Citizen.CreateThread(function()
    for k, v in pairs (Config.Stores) do
        if Config.Stores[k].shopType == "clothing" then
            ...
            AddTextComponentString("Tøjbutik")
		...
		local dist = #(pos - Config.Stores[k].coords)
                if dist < 30 then
                    if not creatingCharacter then
                        ...
                        if dist < 5 then
                            if Config.Stores[k].shopType == "clothing" then
                                 DrawText3Ds(Config.Stores[k].coords.x, Config.Stores[k].coords.y, Config.Stores[k].coords.z + 1.25, '~g~E~w~ - For At Shoppe For Tøj')
                            elseif Config.Stores[k].shopType == "barber" then
                                DrawText3Ds(Config.Stores[k].coords.x, Config.Stores[k].coords.y, Config.Stores[k].coords.z + 1.25, '~g~E~w~ - For At Blive Klippet')
                            elseif Config.Stores[k].shopType == "surgeon" then
                                DrawText3Ds(Config.Stores[k].coords.x, Config.Stores[k].coords.y, Config.Stores[k].coords.z + 1.25, '~g~E~w~ - For At Få Plastik Operation')
                            end
                            if IsControlJustPressed(0, 38) then -- E
                                ...
                                elseif Config.Stores[k].shopType == "surgeon" then
                                    customCamLocation = nil
                                    openMenu({
                                        {menu = "clothing", label = "Funktioner", selected = true},
                                    })
                                end
                            end
                        end
                            ...
                            if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                ...
                                if PlayerData.gang.name == Config.ClothingRooms[k].requiredJob then
                                    ...
                                    if IsControlJustPressed(0, 38) then -- E
                                        ...
                                        QBCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
                                            openMenu({
                                                {menu = "roomOutfits", label = "Outfits", selected = true, outfits = Config.Outfits[PlayerData.gang.name][gender]},
                                                {menu = "myOutfits", label = "Mine Outfits", selected = false, outfits = result},
                                                {menu = "character", label = "Tøj", selected = false},
                                                {menu = "accessoires", label = "Tilbehør", selected = false}
                                            })
...
AddEventHandler('qb-clothing:client:openMenu', function()
    ...
    openMenu({
        ...
        {menu = "clothing", label = "Funktioner", selected = false},
        {menu = "accessoires", label = "Tilbehør", selected = false}
    })
end)
...
RegisterNUICallback('TrackerError', function()
    QBCore.Functions.Notify("Du kan ikke fjerne dit ankelarmbånd ..", "error")
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    ...
    QBCore.Functions.Notify("Du har slettet dit"..data.outfitName.." outfit!")
end)
...
AddEventHandler('qb-clothing:client:loadOutfit', function(oData)
    ...
    if oData.outfitName ~= nil then
        QBCore.Functions.Notify("Du har valgt "..oData.outfitName.."! Tryk på Bekræft for at bekræfte outfit.")
    end
end)
