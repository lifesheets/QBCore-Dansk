...
RegisterNetEvent("qb-bossmenu:server:withdrawMoney", function(amount)
	...
	if not Player.PlayerData.job.isboss then ExploitBan(src, 'withdraw Money Exploiting') return end

	local job = Player.PlayerData.job.name
	if RemoveMoney(job, amount) then
		Player.Functions.AddMoney("cash", amount, 'Boss Menu Hævning')
		TriggerEvent('qb-log:server:CreateLog', 'bossmenu', 'Withdraw Money', "blue", Player.PlayerData.name.. "Withdrawal $" .. amount .. ' (' .. job .. ')', false)
		TriggerClientEvent('QBCore:Notify', src, "Du har hævet: " ..amount.. " DKK", "success")
	else
...
RegisterNetEvent("qb-bossmenu:server:depositMoney", function(amount)
	...
	if not Player.PlayerData.job.isboss then ExploitBan(src, 'Udbetal Penge Exploiting') return end
	...
	if Player.Functions.RemoveMoney("cash", amount) then
		...
		TriggerClientEvent('QBCore:Notify', src, "Du Har Udbetalt: " ..amount.. " DKK", "success")
	else
		TriggerClientEvent('QBCore:Notify', src, "Du har ikke penge nok til at tilføje!", "error")
	end
...
RegisterNetEvent('qb-bossmenu:server:GradeUpdate', function(data)
	...
	if Employee then
		if Employee.Functions.SetJob(Player.PlayerData.job.name, data.grado) then
			TriggerClientEvent('QBCore:Notify', src, "Promoverede Personen!", "success")
			TriggerClientEvent('QBCore:Notify', Employee.PlayerData.source, "Du er blevet promoveret til " ..data.nomegrado..".", "success")
		else
			TriggerClientEvent('QBCore:Notify', src, "Denne grad eksisterer ikke.", "error")
		end
	else
		TriggerClientEvent('QBCore:Notify', src, "Borger er ikke i byen.", "error")
	end
	TriggerClientEvent('qb-bossmenu:client:OpenMenu', src)
end)
	...
	if Employee then
		if target ~= Player.PlayerData.citizenid then
			if Employee.Functions.SetJob("unemployed", '0') then
				...
				TriggerClientEvent('QBCore:Notify', src, "Fyrede Medarbejder!", "success")
				TriggerClientEvent('QBCore:Notify', Employee.PlayerData.source , "Du er blevet fyret.", "error")
			else
...
RegisterNetEvent('qb-bossmenu:server:HireEmployee', function(recruit)
	...
	if Target and Target.Functions.SetJob(Player.PlayerData.job.name, 0) then
		TriggerClientEvent('QBCore:Notify', src, "Du hyrede " .. (Target.PlayerData.charinfo.firstname .. ' ' .. Target.PlayerData.charinfo.lastname) .. " come " .. Player.PlayerData.job.label .. "", "success")
		TriggerClientEvent('QBCore:Notify', Target.PlayerData.source , "Du blev hyret som " .. Player.PlayerData.job.label .. "", "success")
