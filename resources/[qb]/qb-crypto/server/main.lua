    ...
    if crypto ~= nil then
        if Crypto.Worth[crypto] ~= nil then
            ...
            if NewWorth ~= nil then
                ...
                TriggerClientEvent('QBCore:Notify', src, "Du har værdien af "..Crypto.Labels[crypto].."tilpasset fra: ("..Crypto.Worth[crypto].." DKK til: "..NewWorth.." DKK) ("..ChangeLabel.." "..PercentageChange.."%)")
                ...
                TriggerClientEvent('QBCore:Notify', src, "Du har ikke givet en ny værdi .. Nuverende værdier: "..Crypto.Worth[crypto])
            end
        else
            TriggerClientEvent('QBCore:Notify', src, "Denne krypto eksisterer ikke :(, tilgængelige: Bit")
        end
    else
        TriggerClientEvent('QBCore:Notify', src, "Du har ikke angivet Krypto, tilgængelige: Bit")
    end
end, "admin")

QBCore.Commands.Add("checkcryptoworth", "", {}, false, function(source)
    local src = source
    TriggerClientEvent('QBCore:Notify', src, "Bits har en værdi på: "..Crypto.Worth["bit"].." DKK")
end)

QBCore.Commands.Add("crypto", "", {}, false, function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local MyPocket = math.ceil(Player.PlayerData.money.crypto * Crypto.Worth["bit"])

    TriggerClientEvent('QBCore:Notify', src, "Du har: "..Player.PlayerData.money.crypto.." Bits, med en værdi på: "..MyPocket.." DKK")
end)
...
RegisterServerEvent('qb-crypto:server:ExchangeSuccess', function(LuckChance)
    ...
    if ItemData ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', src, "Du har byttet din Cryptostick for: "..Amount.." Bits", "success", 3500)
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["cryptostick"], "remove")
        TriggerClientEvent('qb-phone:client:AddTransaction', src, Player, {}, "Der er "..Amount.." Bits krediteret!", "Kredit")
    end
end)
...
QBCore.Functions.CreateCallback('qb-crypto:server:BuyCrypto', function(source, cb, data)
    local Player = QBCore.Functions.GetPlayer(source)
    local total_price = tonumber(data.Coins) * tonumber(Crypto.Worth["bit"])
    if Player.PlayerData.money.bank >= total_price then
        local CryptoData = {
            History = Crypto.History["bit"],
            Worth = Crypto.Worth["bit"],
            Portfolio = Player.PlayerData.money.crypto + tonumber(data.Coins),
            WalletId = Player.PlayerData.metadata["walletid"],
        }
        Player.Functions.RemoveMoney('bank', total_price)
        TriggerClientEvent('qb-phone:client:AddTransaction', source, Player, data, "Du har købt "..tonumber(data.Coins).." Bits!", "Kredit")
...
QBCore.Functions.CreateCallback('qb-crypto:server:SellCrypto', function(source, cb, data)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        TriggerClientEvent('qb-phone:client:AddTransaction', source, Player, data, "Du har solgt "..tonumber(data.Coins).." Bits!", "Afskrivning")
        Player.Functions.AddMoney('bank', tonumber(data.Coins) * tonumber(Crypto.Worth["bit"]))
        cb(CryptoData)
    else
        cb(false)
    end
...
QBCore.Functions.CreateCallback('qb-crypto:server:TransferCrypto', function(source, cb, data)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        if result[1] ~= nil then
            ...
            Player.Functions.RemoveMoney('crypto', tonumber(data.Coins))
            TriggerClientEvent('qb-phone:client:AddTransaction', source, Player, data, "Du har overført "..tonumber(data.Coins).." Bits!", "Afskrivning")
            ...
            if Target ~= nil then
                ...
                TriggerClientEvent('qb-phone:client:AddTransaction', Target.PlayerData.source, Player, data, "Der er "..tonumber(data.Coins).." Bits krediteret!", "Kredit")
            else
    ...
    Citizen.CreateThread(function()
        ...
        while(true) do
            ...
            if type(get_coin_price) == 'number' then
                Crypto.Worth["bit"] = get_coin_price
            else
